...
import projectData from "@/data/projects/project-raanana-residential.json";
import Link from "next/link";
import { Button } from "@/components/ui/Button";

export default function RaananaProjectPage() {
    return (
        <>
            <Header />
            <main className="pt-24">
                    ...
                    <div className="container">
                        ...
                        <div className="grid md:grid-cols-2 gap-8 max-w-3xl mx-auto">
                            {projectData.metrics.map((metric, index) => (
                                <div
                                    key={index}
                                    className="text-center bg-base-elevated p-8 rounded-[var(--radius-lg)]"
                                >
                                    <div className="text-4xl font-extrabold text-accent-primary mb-2">
                                        {metric.value}
                                    </div>
                                    <div className="text-neutral-300">{metric.label}</div>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                {/* CTA */}
                <section className="py-20 border-t-2 border-accent-primary">
                    <div className="container text-center">
                        <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
                            יש לכם פרויקט דומה?
                        </h2>
                        <p className="text-xl text-neutral-300 mb-8">
                            נשמח לשמוע ולעזור לכם להצליח
                        </p>
                        <Link href="/contact">
